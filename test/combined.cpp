...
/*  @short Creates the COMBINED quadratic cardinality and quadratic bound constrained portfolio optimization problem
...
int createCombined(MSKtask_t* task)
{
  ...
  r = MSK_appendcons(*task,1+N+1+2); //return, sum to 1, x^2<=z, x'Qx<=t, and z'z <= k
  ...
  for(int j=0; j<N; j++) {
    // r = MSK_putaij(*task, j+1, j+1, 1); // for linear
  ...
  if(FILEOUTPUT) {
    MSK_writedata(*task, "result/CombinedOriginal.mps");
    MSK_writedata(*task, "result/CombinedOriginal.lp");
  }
  
  MSK_toconic (*task);
  
  if(FILEOUTPUT)
    MSK_writedata(*task, "result/CombinedOriginal2.mps");
